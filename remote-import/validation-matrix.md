...
| mindmap/sample-directory.mindnode | MindNode directory package | tree directory | Accepted when GitHub tree metadata is supported and package stays within normal target | Opens as a directory-style MindNode package with contents.xml and resources | small | ready | Normal-size directory package, about 812 KB. |
...
| packages/obsidian-sample-package.zip | Obsidian-style Markdown Package .zip | blob/raw file | Accepted if package size is within archive limit | Opens Markdown notes with related image assets when supported | small | ready | Copied from DEV fixture; about 6.6 MB with Obsidian vault config, nested Markdown folders, and image assets. |
